function [ x ] = penalty_path2(f,df,~,~,~,~,h,dh,~,x0,iter)
% f: R^n->R objective
...
% h: R^n->R^m equality constraints, i.e. h_i(x)==0 should be satisfied
% dh: R^{mxn} derivative matrix containing the gradients of h_i-s as rows
% x0: arbitrary (not necessarily feasible) starting point
